...
public class MainLoop implements Runnable  {
	...
	//Discovered instance values from Phase 1
	private final ConcurrentHashMap<Integer, DiscoveredInstance> discovered_instances;
	private int highest_discovered_instance;
	...
	this.discovered_instances = new ConcurrentHashMap<>(); //Discovered instance values from Phase 1
    this.highest_discovered_instance = -1;
    }
		...
		if (this.server_state.scheduler.leader(this.server_state.getCurrentBallot ()) == this.server_state.my_id) {
			if (phaseOneDone == false) {
				...
				try {
					...
					Thread.sleep(10000); // dorme 1000 ms = 1 segundo
	...
	public synchronized void doPhaseOne() {
		...
		System.out.println("oaugasiugfaiugfgafsiufi\n");
		...
		PhaseOneBogusProcessor phase_one_processor = new PhaseOneBogusProcessor(this.server_state.scheduler, low_ballot, high_ballot, quorum);
	...
	public synchronized void doMultiPaxosPhaseOne() {
        ...
        if ((ballot > -1) && (this.server_state.scheduler.leader(ballot) == this.server_state.my_id)) {
            ...
            // Criar processor especializado para MultiPaxos
            MultiPaxosPhaseOneProcessor phase_one_processor = 
                new MultiPaxosPhaseOneProcessor(this.server_state.scheduler, ballot, quorum);
            ...
            if (phase_one_processor.getAccepted() == false) {
                ...
                // Armazenar valores descobertos na estrutura
                storeDiscoveredValues(phase_one_processor.getDiscoveredValues());
     ...
     */
    private void storeDiscoveredValues(Map<Integer, DiscoveredValue> discovered) {
        for (Map.Entry<Integer, DiscoveredValue> entry : discovered.entrySet()) {
            int instance = entry.getKey();
            DiscoveredValue value_info = entry.getValue();
            
            if (value_info.value > 0) { // Só armazenar se há valor válido
                discovered_instance_values.put(instance, value_info.value);
                discovered_instance_ballots.put(instance, value_info.ballot);
                
                System.out.println("Stored discovered value: instance=" + instance + 
                                 ", value=" + value_info.value + 
                                 ", ballot=" + value_info.ballot);
            }
        }
